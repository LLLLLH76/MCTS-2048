...
import gym  
import numpy as np
from PyQt5.QtWidgets import (QMainWindow,QWidget,QPushButton,QDialog,QApplication,QDesktopWidget,QLabel,QMessageBox)
...
class Game2048GUI(QMainWindow):
    ...
    def _initUI(self):
        ...
        col = ['grey', 'purple', 'fuchsia', 'blue', 'green', 'crimson', 'yellow', 'orange', 'violet', 'pink', 'red'] * 100
        for num in range(1, 12):
...
def selectAction(env, depth):
    origin_state = copy.deepcopy(env).state
    for _ in range(resource):
        simulate(env, depth)
    for state_N_Q in allStates_N_Q:
        if origin_state == state_N_Q[0]:
            return np.array(state_N_Q[2]).argmax()
    return random.randint(0,3)

def simulate(env ,depth):
    # print(type(env))
    if depth == 0:
        return 0
    allStates=[state_N_Q[0] for state_N_Q in allStates_N_Q]
    if env.state not in allStates:
        allStates_N_Q.append([env.state,[1,1,1,1],[0,0,0,0]])
        return rollOut(env, depth)
    N=Q=[]
    state_N_Q=[]
    for state_N_Q in allStates_N_Q:
        if env.state == state_N_Q[0]:
            N=state_N_Q[1]
            Q=state_N_Q[2]
            break
    action = np.array([ (Q[action] + c*math.sqrt(math.log(sum(N))/N[action])) for action in range(4)]).argmax()
    new_env = copy.deepcopy(env)
    obs, rew, done, info = new_env.step(action)
    q = rew + gamma*simulate(new_env, depth-1)
    state_N_Q[1][action]+=1
    state_N_Q[2][action]+=(( q - Q[action] ) / N[action] )
    return q

def rollOut(env, depth):
    if depth==0:
        return 0
    action = random.randint(0,3)
    obs, rew, done, info = env.step(action)
    return rew + gamma*rollOut(env, depth-1)
...
if __name__ == '__main__':
    ...
    # root = TreeNode(None, env.state, 0, 0, -1)
    while not done:
        # A random agent as example
        # 0, 1, 2, 3 mean up, down, left, right respectively
        # action = random.randint(0, 3)
        allStates_N_Q=[[env.state,[1,1,1,1],[0,0,0,0]]]
        action = selectAction(copy.deepcopy(env), maxDepth)
        print(action)
        time.sleep(1)
        obs, rew, done, info = env.step(action)
        if done:
